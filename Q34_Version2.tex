...
\maketitle
    ...
    % Beginning of Solution Box
     \begin{tcolorbox}[breakable, colback= blue!5!white, colframe= blue!50!black, title=\textbf{Solution: b) $\displaystyle \frac{x}{\sqrt{4-x^2}} - \sin^{-1}\!\left(\frac{x}{2}\right) + C$}]
        ...
        % Step 5.4
        \solutionStep{1}{Use \qdeflink{abs-val}{absolute value}: $\sqrt{u^2}=|u|$}{
        $\begin{WithArrows}
        ...
        $\begin{aligned}[t]
            2^2 &= 4\\
            \deflink{notation}{(\sin\theta)^2 = \sin^2\theta} \quad \text{(shorthand)} \\
...
\Arrow{Use \qdeflink{const-mult}{constant multiple rule}: $\frac{d}{d\theta}(c f(\theta))=c f'(\theta)$}\\[1ex]
...
\end{WithArrows}$
        ...
        \frac{x}{2} &= \sin\theta \\
        \theta &= \qdeflink{inv-sine}{\sin^{-1}}\!\left(\frac{x}{2}\right)
        \end{aligned}$
        }
        
        % Step 12.2
        \solutionStep{1}{Find $\tan\theta$ using \qdeflink{trig-ratios}{basic trig ratios}}{
        $\begin{aligned}[t]
        \sin\theta &= \frac{x}{2}\\
